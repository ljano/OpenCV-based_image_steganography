...
- 暂时想不到了...
- 哥们知道问题出在哪了nnd，我用RGB图像格式来读的图像，操作的时候直接操作的RGB图像里一个像素点三个颜色通道值，直接对这个值进行了±1的调整，但这样扰动过大且有很多不必要的调整，要保持原有图片大小就会很失真，更优的做法应该是做更精细的调整，比如像LSB算法，用颜色通道的最低位来存数据，这样大部分数据其实不会修改原本的通道值，因为只有0/1，一半的概率，这样做的效果会更优。——2024/10/09 17:16
