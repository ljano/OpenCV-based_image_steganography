...
def data_init() -> list:
    ...
    """
    try:
        path_text: np.ndarray = np.array([[str] * 2] * 5)
        path_text[0][0] = r'd:\need\txt\ten\share_all_0_0.txt'
        path_text[0][1] = r'd:\need\txt\ten\share_all_0_1.txt'
        path_text[1][0] = r'd:\need\txt\ten\share_all_1_0.txt'
        path_text[1][1] = r'd:\need\txt\ten\share_all_1_1.txt'
        path_text[2][0] = r'd:\need\txt\ten\share_all_2_0.txt'
        path_text[2][1] = r'd:\need\txt\ten\share_all_2_1.txt'
        path_text[3][0] = r'd:\need\txt\ten\share_all_3_0.txt'
        path_text[3][1] = r'd:\need\txt\ten\share_all_3_1.txt'
        path_text[4][0] = r'd:\need\txt\ten\share_all_4_0.txt'
        path_text[4][1] = r'd:\need\txt\ten\share_all_4_1.txt'

        data_raw = [[e.get_text(path_text[i][j]) for j in range(2)] for i in range(5)]
...
def start_ten(path_raw, path_save):
    """
    将path_raw中的数图片拷贝十份，然后将十份数据分别写入，再初步压缩，得到十张图片，分别存储于对应路径path_save(见compress_all)。
    ...
    try:
        img = [cv.imread(path_raw, cv.IMREAD_UNCHANGED) for i in range(10)]
    except IOError as err:
        print(f"没有找到文件或读取文件失败，图片路径不要有中文哦。Error is {err}")
    else:
        print("读取图片成功！")
        pix_raw: np.ndarray = np.array([[int] * 2] * 5)
        pix_new: np.ndarray = np.array([[int] * 2] * 5)
        img_new = [[np.ndarray([1])] * 2] * 5
        for i in range(5):
            for j in range(2):
...
if __name__ == '__main__':
    path_png_save1 = [["d:/need/all_picture/test/share_{}_{}.png".format(i, j) for j in range(2)]
                      for i in range(5)]
    path_jpg_save1 = [["d:/need/all_picture/test/share_{}_{}.jpg".format(i, j) for j in range(2)]
                      for i in range(5)]
    path_jpg_raw1 = r'd:/need/all_picture/test.jpg'
    start_ten(path_jpg_raw1, path_png_save1)
